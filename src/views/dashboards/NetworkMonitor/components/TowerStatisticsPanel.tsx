    ...
    <div className="bg-white border border-gray-200 rounded-lg">
      ...
      >
        <div className="flex items-center">
          ...
          <h2 className="text-lg font-bold text-gray-900">{selectedTower.name}</h2>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 animate-fadeIn space-y-4">
              ...
              <div className="rounded-lg overflow-hidden bg-gray-50 border border-gray-200 p-2">
                {selectedTower.failureHistory.map((failure, index) => (
                  <div key={index} className="flex items-center justify-between py-2 border-b border-gray-200 last:border-0">
                    ...
                    <div className="text-xs text-gray-600">{failure.reason}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button className={`${selectedTower.status === 'failed' ? 'bg-error hover:bg-error/90' : 'bg-gray-500 hover:bg-gray-600'
